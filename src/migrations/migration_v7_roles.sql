-- Migration v7: Connect Roles and Safeguard Votes

-- 1. Add Role to Users
alter table public.users 
add column if not exists role text default 'Student';

-- 2. Update handle_vote to prevent negative counts
...
begin
  ...
  if existing_vote = new_vote_type then
    ...
    -- Decrement Document Counter (SAFEGUARDED)
    if new_vote_type = 1 then
      update public.documents set upvote_count = greatest(0, upvote_count - 1) where id = doc_id;
      -- Revert Owner Rep (+5 was given, so -5)
      update public.users set total_upvotes = greatest(0, total_upvotes - 1), reputation_score = reputation_score - 5 where id = owner_id;
    else
      update public.documents set downvote_count = greatest(0, downvote_count - 1) where id = doc_id;
      -- Revert Owner Rep (-2 was given, so +2)
      update public.users set total_downvotes = greatest(0, total_downvotes - 1), reputation_score = reputation_score + 2 where id = owner_id;
    end if;
    ...
    if new_vote_type = 1 then
      -- Was Down (-1), Now Up (1)
      update public.documents set downvote_count = greatest(0, downvote_count - 1), upvote_count = upvote_count + 1 where id = doc_id;
      -- Owner: Revert Down (- -2 = +2) AND Apply Up (+5) = +7 total
      update public.users 
      set total_downvotes = greatest(0, total_downvotes - 1), total_upvotes = total_upvotes + 1, reputation_score = reputation_score + 7 
      where id = owner_id;
    else
      -- Was Up (1), Now Down (-1)
      update public.documents set upvote_count = greatest(0, upvote_count - 1), downvote_count = downvote_count + 1 where id = doc_id;
      -- Owner: Revert Up (-5) AND Apply Down (-2) = -7 total
      update public.users 
      set total_upvotes = greatest(0, total_upvotes - 1), total_downvotes = total_downvotes + 1, reputation_score = reputation_score - 7 
      where id = owner_id;
    end if;
